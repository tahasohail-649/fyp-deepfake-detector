...
def build_model(model_name: str = "efficientnet_b0", pretrained: bool = True) -> nn.Module:
...
def train(args):
    logger = setup_logger(f"efficientnet_b0")
    ...
    logger.info("Model: EfficientNet-B0 (pretrained ImageNet)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            patience_counter = 0
            ckpt_path = os.path.join(CHECKPOINT_DIR, "efficientnet_b0_best.pth")
            torch.save({
                "epoch": epoch,
    ...
    hist_path = os.path.join(LOG_DIR, "efficientnet_b0_history.json")
