...
def build_model(pretrained: bool = True) -> nn.Module:
    # xception in timm is 'xception' or 'legacy_xception'
...
def train(args):
    ...
    logger.info(f"Model: Xception (pretrained ImageNet) | Params: {n_params:.1f}M")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            patience_counter = 0
            ckpt_path = os.path.join(CHECKPOINT_DIR, "xception_best.pth")
            torch.save({
                "epoch": epoch,
    ...
    hist_path = os.path.join(LOG_DIR, "xception_history.json")
